...
def optimize_similarity_matrix(kernels, weights, L, S):
    ...
    for l, k_mat in enumerate(kernels):
        weighted_kernels += weights[l] * k_mat
    coefficients = weighted_kernels + label_term

    objective_func = -cvxpy.sum(cvxpy.multiply(coefficients, S)) + beta * cvxpy.norm(S, "fro")**2
    objective = cvxpy.Minimize(objective_func)
    constraints = [
        cvxpy.sum(S, axis=1) == 1,
        S >= 0
    ]
